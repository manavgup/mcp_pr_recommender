...
from mcp_pr_recommender.src.lib.models.recommender_models import (
    PRGroupingStrategy, PRGroup, GroupingStrategyType, PatternAnalysisResult,
    GroupingStrategyDecision
)
...
class BatchProcessorTool(BaseRepoTool):
    ...
    def _run(
        ...
        try:
            ...
            if not cleaned_batch_output:
                raise ValueError("BatchSplitterOutput JSON is invalid or empty after cleaning.")
            if not cleaned_strategy_decision:
                raise ValueError("GroupingStrategyDecision JSON is invalid or empty after cleaning.")
            if not cleaned_repo_analysis:
                raise ValueError("RepositoryAnalysis JSON is invalid or empty after cleaning.")
            ...
            strategy_type_str: str = getattr(strategy_decision, 'strategy_type', 'mixed')
            ...
            for i, current_batch_paths in enumerate(list_of_all_batches):
                ...
                batch_explanation = f"Error processing batch {i}."
                batch_complexity = 1.0
                ungrouped_in_batch: List[str] = current_batch_paths
                        ...
                        if not batch_files_objects:
                            ...
                            for group in batch_groups:
                                ...
                                if not group.files:
                                    continue
                                if not group.title:
                                    group.title = f"Chore: Grouped changes (Batch {i})"
                                if not group.rationale:
                                    group.rationale = f"Group created by {strategy_type.value} strategy for batch {i}."
                ...
                all_batch_results.append(batch_strategy_result.model_dump(mode='json'))
    ...
    def _group_by_size(self, batch_files_objects: List[FileChange]) -> List[PRGroup]:
        ...
        if num_files == 0:
            return []
        num_groups = 1 if num_files <= 10 else 2
    ...
    def _generate_branch_name(self, title: str) -> str:
        ...
        if not branch_name:
            branch_name = 'fix-unnamed-changes'
    ...
    def _generate_pr_description(self, group: PRGroup, batch_files_context: List[FileChange]) -> str:
        ...
        for file_path in sorted(group.files)[:15]:
            ...
            status_char = "?"
            if fc:
                st_val = getattr(fc, 'staged_status', None)
                ut_val = getattr(fc, 'unstaged_status', None)
                if st_val and st_val != " ":
                    status_char = st_val
                elif ut_val and ut_val != " ":
                    status_char = ut_val
                else:
                    status_char = ' '
    ...
    def _generate_strategy_explanation(self, strategy_value: str, groups: List[PRGroup], batch_file_count: int, batch_index: int) -> str:
        ...
        grouped_files_count = sum(len(g.files) for g in groups if g.files)
        strategy_name = strategy_value.replace("_", " ").title()
        explanation = (f"Batch {batch_index}: Applied **{strategy_name}** grouping to {batch_file_count} files. Resulted in {group_count} group(s) covering {grouped_files_count} files. ")
        return explanation
